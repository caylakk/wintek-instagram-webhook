...
const WHATSAPP_NUMBER_DISPLAY = "+90 272 216 48 48";
const WHATSAPP_NUMBER_DIGITS = "902722164848";
...
// --- Teknik destek / Iade-Garanti formlari ------------------------------------
// Formlar doldurulunca talep Redis'e kaydedilir, Telegram'a bildirim gider ve
// Brevo uzerinden MAIL_TO adresine mail atilir. Brevo API anahtari Render'da
// BREVO_API_KEY adiyla durur; gonderen adres (MAIL_FROM) Brevo'da onayli olmali.
const DESTEK_FORM_URL = `${PUBLIC_URL}/teknik-destek`;
const IADE_FORM_URL = `${PUBLIC_URL}/iade-garanti`;
const FORM_REQUESTS_KEY = "formlar:talepler";
const FORM_REQUESTS_MAX = 500;
...
const WA_MENU_DEALER = "WA_MENU_DEALER";
...
FORMLAR (talepler bu formlardan doğrudan ekibimize ve info@wintekgroup.com.tr adresine iletilir):
- Müşteri bir ürünle ilgili arıza, sorun, kullanım problemi yaşıyorsa ya da teknik destek istiyorsa: kısa bir anlayış cümlesi kur ve teknik destek formunu öner. Link: ${DESTEK_FORM_URL}
- İade, değişim, garanti, hasarlı/hatalı ürün ya da yanlış gönderim gibi konularda: iade/garanti formunu öner. Link: ${IADE_FORM_URL}
- Bayilik başvurusu isteyenlere bayilik formunu öner. Link: ${BAYILIK_FORM_URL}
- Linki olduğu gibi, ayrı bir satırda yaz. İade/garanti koşulları, süreleri veya prosedürleri hakkında bilgi UYDURMA; formu doldurduktan sonra ekibimizin kendilerine dönüş yapacağını söyle.`;
...
const FORM_PAGE_STYLE = `
    ...
    .req { color: #d32f2f; }
    .consent { display: flex; gap: 8px; align-items: flex-start; font-weight: normal; font-size: 0.9em; color: #444; margin-top: 18px; }
...
function renderFormPage({ title, intro, action, fieldsHtml, buttonText, query }) {
    ...
    <head>
    <meta charset="UTF-8">
    <title>${escapeHtml(title)} - Wintek</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    ...
    ${fieldsHtml}
    <label class="consent"><input type="checkbox" name="onay" value="1" required> <span>Bilgilerimin bu talep kapsamında benimle iletişime geçilmesi amacıyla işlenmesini kabul ediyorum. <span class="req">*</span></span></label>
...
function renderFormThanksPage(title, message) {
    return `<!DOCTYPE html>
    <html lang="tr">
    <head>
    <meta charset="UTF-8">
    <title>${escapeHtml(title)} - Wintek</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <style>body { font-family: -apple-system, Segoe UI, Roboto, Arial, sans-serif; max-width: 480px; margin: 80px auto; padding: 0 20px; line-height: 1.6; color: #222; text-align: center; } h1 { font-size: 1.4em; }</style>
    </head>
    <body>
    <h1>✅ ${escapeHtml(title)}</h1>
...
async function sendFormEmail(subject, fields, replyToEmail, replyToName) {
    ...
    const html = `<div style="font-family:Arial,sans-serif;font-size:14px;color:#222">
        <h2 style="color:#d32f2f;margin:0 0 12px">${escapeHtml(subject)}</h2>
        <table style="border-collapse:collapse">${rows}</table>
        <p style="color:#777;font-size:12px;margin-top:16px">Bu mail Wintek web formundan otomatik olarak gönderildi. Müşteriye cevap vermek için "Yanıtla" demeniz yeterli (e-posta adresini girdiyse).</p>
    </div>`;
    const body = {
    ...
    try {
        ...
        console.log(`Form maili gonderildi -> ${MAIL_TO}: ${subject}`);
        return true;
    } catch (err) {
        console.error("Form maili gonderilemedi:", err.response?.status || "", err.response?.data || err.message);
        return false;
    }
}
...
async function handleFormSubmission({ type, emoji, title, fields, adSoyad, eposta, kanal, musteri }) {
    ...
    if (redis) {
        try {
            await redis.lpush(FORM_REQUESTS_KEY, JSON.stringify(record));
            await redis.ltrim(FORM_REQUESTS_KEY, 0, FORM_REQUESTS_MAX - 1);
        } catch (err) {
            console.error("Form talebi Redis'e kaydedilemedi:", err.message);
        }
    }
    const kanalText = kanal && kanal !== "web" ? ` (${kanal})` : "";
    notifyAdmin(
        `${emoji} <b>Yeni ${escapeHtml(title)}${escapeHtml(kanalText)}</b>\n` +
        fields.filter(([, v]) => v).map(([k, v]) => `${escapeHtml(k)}: ${escapeHtml(v)}`).join("\n") +
        (musteri ? `\nMüşteri kimliği: ${escapeHtml(musteri)}` : "")
    );
    await sendFormEmail(
        `${title} - ${adSoyad}`,
...
app.get("/teknik-destek", (req, res) => {
    ...
    res.send(renderFormPage({
        ...
        intro: "Ürününüzle ilgili yaşadığınız sorunu aşağıya yazın, teknik ekibimiz en kısa sürede size dönüş yapsın.",
    ...
    <label for="firma">Firma / İşletme Adı</label>
    ...
    <textarea name="sorun" id="sorun" placeholder="Ürünü nerede ve nasıl kullanıyorsunuz, ne tür bir sorun yaşıyorsunuz?" required></textarea>`,
    }));
});

app.post("/teknik-destek", async (req, res) => {
    if (req.body.website) {
        res.send(renderFormThanksPage("Talebiniz alındı", "Teşekkürler."));
        return;
    }
    const adSoyad = clean(req.body.adSoyad, 120);
    const firma = clean(req.body.firma, 120);
    const telefon = clean(req.body.telefon, 40);
    ...
    await handleFormSubmission({
        ...
        fields: [["Ad Soyad", adSoyad], ["Firma", firma], ["Telefon", telefon], ["E-posta", eposta], ["Ürün", urun], ["Sorun", sorun]],
        adSoyad,
        eposta,
    ...
    res.set("Content-Type", "text/html; charset=utf-8");
    res.send(renderFormThanksPage("Talebiniz alındı", `Teşekkürler ${adSoyad}, teknik destek talebiniz bize ulaştı. Teknik ekibimiz en kısa sürede sizinle iletişime geçecek.`));
});

app.get("/iade-garanti", (req, res) => {
    res.set("Content-Type", "text/html; charset=utf-8");
    res.send(renderFormPage({
        title: "İade / Garanti Formu",
        intro: "İade, değişim veya garanti talebiniz için aşağıdaki bilgileri doldurun, ekibimiz en kısa sürede size dönüş yapsın.",
        action: "/iade-garanti",
        buttonText: "Talebi Gönder",
        query: req.query,
        fieldsHtml: `
    ...
    <label for="tarih">Satın Alma Tarihi</label>
...
app.post("/iade-garanti", async (req, res) => {
    ...
    const telefon = clean(req.body.telefon, 40);
    const eposta = clean(req.body.eposta, 120);
    const talepTuru = clean(req.body.talepTuru, 20);
...
// "Teknik Destek" ve "İade/Garanti" butonlari AI'ya gitmeden ilgili online forma
// (ve alternatif olarak WhatsApp'a) yonlendiriyor. Form doldurulunca talep
// info@ adresine mail olarak ve Telegram'a bildirim olarak duser.
if (postback?.payload === "QR_SUPPORT") {
    await sendDirectReplyWithButtons(
        senderId,
        "Teknik destek talebinizi kısa formdan iletebilirsiniz, teknik ekibimiz en kısa sürede size dönüş yapar. Dilerseniz WhatsApp'tan da yazabilirsiniz:",
        [
            { type: "web_url", url: `${DESTEK_FORM_URL}?kanal=instagram&musteri=${encodeURIComponent(senderId)}`, title: "Destek Formu" },
            { type: "web_url", url: WHATSAPP_LINK, title: "WhatsApp'tan Yaz" },
        ]
    );
    recordResponseTime("dm", receivedAt);
...
if (postback?.payload === "QR_RETURN") {
    await sendDirectReplyWithButtons(
        senderId,
        "İade, değişim veya garanti talebinizi kısa formdan iletebilirsiniz, ekibimiz en kısa sürede size dönüş yapar. Dilerseniz WhatsApp'tan da yazabilirsiniz:",
        [
            { type: "web_url", url: `${IADE_FORM_URL}?kanal=instagram&musteri=${encodeURIComponent(senderId)}`, title: "İade/Garanti Formu" },
            { type: "web_url", url: WHATSAPP_LINK, title: "WhatsApp'tan Yaz" },
        ]
    );
    recordResponseTime("dm", receivedAt);
...
// Karsilama menusundeki "Teknik Destek / Iade-Garanti / Bayilik" butonlari:
// AI'ya gitmeden ilgili formun linkini "Formu Ac" butonuyla gonderiyoruz.
if ([WA_MENU_SUPPORT, WA_MENU_RETURN, WA_MENU_DEALER].includes(buttonReply?.id)) {
    if (await isDuplicateEvent(dedupKey)) return;
    const q = `?kanal=whatsapp&musteri=${encodeURIComponent(from)}`;
    if (buttonReply.id === WA_MENU_SUPPORT) {
        await sendWhatsAppFormLink(from, "Teknik destek talebinizi bu kısa formdan iletebilirsiniz, teknik ekibimiz en kısa sürede size dönüş yapar. Dilerseniz sorununuzu buraya da yazabilirsiniz.", `${DESTEK_FORM_URL}${q}`);
    } else if (buttonReply.id === WA_MENU_RETURN) {
        await sendWhatsAppFormLink(from, "İade, değişim veya garanti talebinizi bu kısa formdan iletebilirsiniz, ekibimiz en kısa sürede size dönüş yapar.", `${IADE_FORM_URL}${q}`);
    } else {
        await sendWhatsAppFormLink(from, "Bayilik başvurunuzu bu kısa formdan iletebilirsiniz, ekibimiz en kısa sürede sizinle iletişime geçer.", `${BAYILIK_FORM_URL}${q}`);
    }
    console.log(`WhatsApp menu secimi - Numara: ${from}, Secim: ${buttonReply.title || buttonReply.id}`);
    return;
}
...
async function sendWhatsAppInteractive(to, interactive) {
    ...
    try {
        ...
        console.error(`WhatsApp interaktif mesaj gonderilemedi -> ${to}:`, err.response?.data || err.message);
        return false;
    }
}

// Karsilama mesajinin ardindan gelen 3 butonlu menu (WhatsApp en fazla 3 buton,
// buton basligi en fazla 20 karakter kabul ediyor).
async function sendWhatsAppMenu(to) {
    await sendWhatsAppInteractive(to, {
        type: "button",
        body: { text: "Size nasıl yardımcı olabiliriz? Aşağıdan seçebilir ya da sorunuzu doğrudan yazabilirsiniz." },
        action: {
            buttons: [
                { type: "reply", reply: { id: WA_MENU_SUPPORT, title: "Teknik Destek" } },
                { type: "reply", reply: { id: WA_MENU_RETURN, title: "İade / Garanti" } },
                { type: "reply", reply: { id: WA_MENU_DEALER, title: "Bayilik" } },
            ],
        },
    });
}

// "Formu Ac" butonlu link mesaji; olmazsa linki duz metin olarak gonderir.
async function sendWhatsAppFormLink(to, text, formUrl) {
    const ok = await sendWhatsAppInteractive(to, {
        type: "cta_url",
        body: { text },
        action: { name: "cta_url", parameters: { display_text: "Formu Aç", url: formUrl } },
    });
    if (!ok) {
        await sendWhatsAppReply(to, `${text}\n${formUrl}`);
    }
}
